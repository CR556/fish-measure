...
import type { DistanceEvent, TrackingStateEvent } from '../../modules/fish-measure';
...
export function DebugOverlay({ event, tracking, lastError }: Props) {
  ...
  const rows: [string, string][] = [
    ...
    ['mode', event?.mode ?? 'â€”'],
    ['method', event?.method ?? 'â€”'],
    ['raw', event ? `${event.rawMeters.toFixed(3)} m` : 'â€”'],
    ['smoothed', event ? `${event.meters.toFixed(3)} m` : 'â€”'],
    ['confidence', event?.confidence ?? 'â€”'],
    ['rate', hz ? `${hz.toFixed(1)} Hz` : 'â€”'],
