...
/// Native view that owns the ARKit world-tracking session (rear camera only —
/// this app has no TrueDepth mode, so there is no ARSession/AVCaptureSession
/// exclusivity machine to manage).
class FishARView: ExpoView {
  ...
  func setMode(_ newMode: String) {
    guard ["rearTap", "rearCrosshair", "heatmap"].contains(newMode) else {
